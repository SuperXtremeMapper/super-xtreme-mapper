...
nonisolated final class AssistantConversationService: AssistantConversing, Sendable {
    ...
    func decodeResponse(_ data: Data, request: AssistantConversationRequest) throws -> AssistantConversationResponse {
        ...
        // Require the expected keys to be present, but tolerate extra ones the
        // API may add to tool inputs (e.g. "caller"); the decoder ignores them.
        guard tools.count == 1,
              tools[0]["name"] as? String == "return_conversation",
              let input = tools[0]["input"] as? [String: Any],
              input["operations"] != nil,
              let answer = input["answer"] as? [String: Any],
              ["facts", "interpretations", "unknowns"].allSatisfy({ answer[$0] != nil })
        else { throw ServiceError.invalidResponse }
        for key in ["facts", "interpretations"] {
            guard let claims = answer[key] as? [[String: Any]],
                  claims.allSatisfy({ $0["text"] != nil && $0["rowIDs"] != nil })
            else { throw ServiceError.invalidResponse }
        }
