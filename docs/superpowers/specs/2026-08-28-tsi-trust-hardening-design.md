...
Before `fileWrapper` returns, the document records one pending write receipt: generation, exact output bytes, semantic baseline-at-write, and disposition. NSDocument save operations for one document are serialized; the document rejects creation of a second in-flight receipt rather than guessing about notification correlation. The app replaces the `DocumentGroup` Save and Save As commands and routes them, along with close and termination saves, through one completion-aware `DocumentSaveCoordinator`. Autosave remains disabled. A successful completion commits that sole receipt. A failed or cancelled completion discards it; a retry creates a new generation. The success notification may remain as a compatibility signal for dirty-state bookkeeping, but it is not the sole receipt-lifecycle mechanism.
...
- Maximum XML elements: 100,000. An element is counted once for each `XMLParser` `didStartElement` callback; text/comment/processing-instruction callback chunking is not counted.
